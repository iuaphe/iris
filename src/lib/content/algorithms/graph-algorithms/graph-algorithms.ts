...
export default article('Graph Algorithms', ({ figMan, algMan }) => {
	...
	return [
		...
		// 		math([`\\sum_{i=1}^{n} (n - i) = \\sum_{j=0}^{n-1} j = \\frac{n(n-1)}{2}`]),
		...
		// 		math([`\\sum_{v \\in V} \\text{deg}(v) = 2|E|`]),
		...
		// 			p`We can compute this power by repeatedly multiplying $G$ by itself. Each of the $k - 1$ multiplications take $\\Theta(n^3)$ time, so the algorithm takes $\\Theta(n^3k)$ time. We can speed this up by using the same trick we used to speed up integer exponentiation. Since matrix multiplication is associative, we can rewrite $W$ as the product of two matrices with approximately half the number of multplications. If $k$ is even, we can rewrite $W$ as`,

		// 			math([`W = G^k = (G^{k/2})(G^{k/2})`]),

		// 			p`If $k$ is odd,`,

		// 			math([`W = G^k = (G^{\\lfloor k/2 \\rfloor})(G^{\\lfloor k/2 \\rfloor})G`]),

		// 			p`Either way, we can recursively compute $G^{\\lfloor k/2 \\rfloor}$ and perform a constant number of matrix multiplications, which each take $\\Theta(n^3)$ time. Therefore, this algorithm has the recurrence`,

		// 			math([`T(k) = cn^3 + 2T(k/2)`]),

		// 			p`Note that $k$ varies in our recursive calls, but $n$ always stays the same. Using familiar techniques, we can show that $T(n) \\in \\Theta(n^3\\log k)$, which is a massive improvement. The main point is that by interpreting our graph as a familiar mathematical object (a matrix), we were able to reduce our problem of counting walks to a more familiar problem of matrix exponentiation, which we know how to do efficiently.`
		// 		]),
		...
		// 		p`So far, we have only answered local questions about graphs:`,
		...
		// 		ul([
		// 			p`If your graph represents a set of locations and roadways between them, this problem asks whether there is a (literal, physical) path between two locations.`,
		// 			p`If your graph represents a social network (with vertices as people and edges as personal connections), this problem asks whether there is a line of connections between two people — this would tell you, for instance, whether a rumor could spread between these two people just by word of mouth.`,
		// 			p`If your graph represents states and transitions between them, this problem asks whether there is a way to get from one state to another only through transitions.`
		// 		]),
		...
		// 		math([`p = \\langle u, p_1, \\dots, p_{n-1}, w \\rangle`]),

		// 		p`If we construct a new path by simply appending the edge $(w, v)$, we get the path`,

		// 		math([`p' = \\langle u, p_1, \\dots, p_{n-1}, w, v \\rangle`]),
		...
		p`First, a **spanning tree** $T$ of a graph $G$ is a subgraph of $G$ such that the following properties hold:`,

		ul([
			p`$T$ is connected and acyclic; that is, $T$ is a **tree**.`,
			p`$T$ contains every vertex of $G$.`
		]),

		p`In essence, a spanning tree of a graph gives us a way to connect every vertex with every other vertex in the minimum number of edges. Further, let a **minimum spanning tree** of a graph be defined as a spanning tree which has minimum possible total weight. Notice that I said "a" and not "the" minimum spanning tree -- there may be more than one. Such a tree is useful because it gives us a set of edges that we may use to get from any node to any other node with the minimum amount of total weight. If weight represents a resource, like meters of wire, minimizing that resource is a good idea.`,
		...
		fig(
			...
			Prims,
			"Prim's Algorithm",
			"Prim's algorithm for computing the minimum spanning tree of a graph. Of all pink fringe edges, the one with minimum weight is added to the tree. Left click to advance the algorithm."
		),
		...
		p`**Theorem (The Spanning Tree Lemma).** Suppose $T$ is a spanning tree of a graph $G$. Let $e$ be an edge in $G \\setminus T$. Then the following are true:`,
		ul([
			p`$T \\cup \\{e\\}$ contains a unique cycle $C$.`,
